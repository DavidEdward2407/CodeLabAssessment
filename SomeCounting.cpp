/////////////////////////////////////////
/////////DAVID EDWARD ZUMERLE////////////
/////////STUDENT ID: 575808//////////////
///////COMPUTING YEAR 1 SEM 1////////////
/////////////////////////////////////////
/*Use your newly acquired knowledge of the for loop to complete the following tasks. Print all values to console in each case.

Write a loop that counts up from 0 to 50 in increments of 1.
Write a loop that counts down from 50 to 0 in decrements of 1.
Write a loop that counts up from 30 to 50 in increments of 1.
Write a loop that counts down from 50 to 10 in decrements of 2.
Write a loop that counts up from 100 to 200 in increments of 5.
You may include all loops in a single project*/
#include <iostream>
using namespace std;

int main() {
    // 1. Count up from 0 to 50 in increments of 1
    cout << "Counting up from 0 to 50:" << endl;
    for (int i = 0; i <= 50; i++) {
        cout << i << " ";
    }
    cout<<endl;

    // 2. Count down from 50 to 0 in decrements of 1
    cout << "\nCounting down from 50 to 0:" << endl;
    for (int i = 50; i >= 0; i--) {
        cout << i << " ";
    }
    cout<<endl;

    // 3. Count up from 30 to 50 in increments of 1
    cout << "\nCounting up from 30 to 50:" << endl;
    for (int i = 30; i <= 50; i++) {
        cout << i << " ";
    }
    cout<<endl;

    // 4. Count down from 50 to 10 in decrements of 2
    cout << "\nCounting down from 50 to 10 in steps of 2:" << endl;
    for (int i = 50; i >= 10; i -= 2) {
        cout << i << " ";
    }
    cout<<endl;

    // 5. Count up from 100 to 200 in increments of 5
    cout << "\nCounting up from 100 to 200 in steps of 5:" << endl;
    for (int i = 100; i <= 200; i += 5) {
        cout << i << " ";
    }
    cout<<endl;

    return 0;
}
